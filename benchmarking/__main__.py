...
def knowledge_benchmark():
    ...
    namespace_pathname = "benchmark2_" + uuid.uuid1().hex[:16]

    p = pipeline.Pipeline()
    p.add_task(init.InitCorpora("localhost", 8010, namespace_pathname, ["knowledge"]))
    p.add_task(dataloader.DownloadWikipedia())
    load_task_id = p.add_task(dataloader.LoadWikipedia(corpus_pathname=f"{namespace_pathname}:knowledge"))
    query_task_id = p.add_task(query.QueryWithClueList(namespace_pathname, knowledge_queries))
